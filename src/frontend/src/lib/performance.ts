    ...
    const rating =
      startupTime < 1000
        ...
        : startupTime < 2500
          ...
          : "🔴 Needs Improvement";
    console.log(
      `[Performance] 🚀 ${appName} startup: ${startupTime.toFixed(2)}ms ${rating}`,
    );
  }
}
...
export function logPerformanceSummary(): void {
  ...
  if (metrics.vitals.LCP) {
    ...
    console.log(
      `${icon} LCP (Largest Contentful Paint): ${value.toFixed(2)}ms - ${rating}`,
    );
  }
  ...
  if (metrics.vitals.FID) {
    ...
    const icon =
      rating === "good" ? "🟢" : rating === "needs-improvement" ? "🟡" : "🔴";
    console.log(
      `${icon} FID (First Input Delay): ${value.toFixed(2)}ms - ${rating}`,
    );
  }
  ...
  if (metrics.vitals.CLS) {
    ...
    const icon =
      rating === "good" ? "🟢" : rating === "needs-improvement" ? "🟡" : "🔴";
    console.log(
      `${icon} CLS (Cumulative Layout Shift): ${value.toFixed(4)} - ${rating}`,
    );
  }
  ...
  if (metrics.vitals.FCP) {
    ...
    const icon =
      rating === "good" ? "🟢" : rating === "needs-improvement" ? "🟡" : "🔴";
    console.log(
      `${icon} FCP (First Contentful Paint): ${value.toFixed(2)}ms - ${rating}`,
    );
  }
  ...
  if (metrics.vitals.TTFB) {
    ...
    const icon =
      rating === "good" ? "🟢" : rating === "needs-improvement" ? "🟡" : "🔴";
    console.log(
      `${icon} TTFB (Time to First Byte): ${value.toFixed(2)}ms - ${rating}`,
    );
  }
  ...
  if (metrics.vitals["DOM Ready"]) {
    console.log(
      `⚡ DOM Ready: ${metrics.vitals["DOM Ready"].value.toFixed(2)}ms`,
    );
  }

  if (metrics.vitals["Page Load"]) {
    console.log(
      `📦 Page Load: ${metrics.vitals["Page Load"].value.toFixed(2)}ms`,
    );
  }
  ...
  try {
    ...
    channel.port1.onmessage = (event) => {
      if (event.data.type === "CACHE_SIZE") {
        ...
        for (const cache of event.data.sizes as {
          name: string;
          size: number;
        }[]) {
